...
const INSCRIPTION_TEXT_FILTERATE:[u8;14] = [123,34,112,34,58,34,98,114,99,45,50,48,34,44];
...
define_table! { LAST_SCAN_INSCRIPTION_NUMBER, u64, u64 }
define_table! { NUMBER_TO_TEXT_INSCRIPTION, u64, &InscriptionIdValue }
...
pub(crate) struct TextInscriptionsInfo {
    id: String,
    number: u64,
    address:String,
    genesis_fee: u64,
...
impl Index {
  pub(crate) fn open(options: &Options) -> Result<Self> {
    ...
    let database = match unsafe { Database::builder().open_mmapped(&path) } {
      ...
      Err(redb::Error::Io(error)) if error.kind() == io::ErrorKind::NotFound => {
        ...
        tx.open_table(LAST_SCAN_INSCRIPTION_NUMBER)?;
  ...
  //M 
  pub (crate) fn text_inscriptions_list(
    &self,
    page:u64,
    size:u64
  ) -> Result<Vec<TextInscriptionsInfo>> {
    let rtx = self.database.begin_read()?;
    let index = (page-1)*size;
    let table = rtx.open_table(NUMBER_TO_TEXT_INSCRIPTION)?;
    let brc20_inscription:Vec<InscriptionId> = table
      .range(index..)?
      .rev()
      .take(size as usize)
      .map(|(_number, id)| Entry::load(*id.value()))
      .collect();
    let mut list:Vec<TextInscriptionsInfo> = Vec::new();
    for inscription_id in brc20_inscription{
      let info = self.get_inscription_api_info(inscription_id);
      match info {
        Ok(value) =>{
          list.push(value);
        },
  ...
  //M 
  pub (crate) fn get_inscription_api_info(
    &self,
    inscription_id:InscriptionId,
  ) -> Result<TextInscriptionsInfo> {
    ...
    let res:TextInscriptionsInfo = TextInscriptionsInfo{
      id: inscription_id.to_string(),
      number: entry.number,
      address: address,
      genesis_fee: entry.fee,
      genesis_height: entry.height,
      genesis_hash: hash.block_hash().to_string(),
      ...
      sat_ordinal: "".to_string(),
      sat_rarity: "".to_string(),
  ...
  //M 
  pub (crate) fn get_inscription_number(
    &self
  ) -> Result<u64> {
    let rtx = self.database.begin_read()?;
    let tb = rtx.open_table(INSCRIPTION_NUMBER_TO_INSCRIPTION_ID)?;
    let latest = match tb.iter()?.rev().next() {
  ...
  //M 
  pub (crate) fn update_brc_number(
    &self,
    num:u64,
  ) -> Result<()> {
    let wtx = self.begin_write()?;
    wtx.open_table(LAST_SCAN_INSCRIPTION_NUMBER)?
      .insert(0, num)?;
    wtx.commit()?;
    Ok(())
  }
  //M 
  pub (crate) fn get_brc_last_number(
    &self
  ) -> Result<u64> {
    let rtx = self.database.begin_read()?;
    let tb = rtx.open_table(LAST_SCAN_INSCRIPTION_NUMBER);
    let mut num:u64 = 0;
    match tb {
      Ok(t) => {
        match t.get(0)?{
      ...
      Err(_) =>{
        self.update_brc_number(0).unwrap();
      }
    }
  ...
  //M  
  pub (crate) fn get_text_inscriptions_number(
    &self
  ) -> Result<u64> {
    let rtx = self.database.begin_read()?;
    let mut num = 0;
    let number_brc = rtx.open_table(NUMBER_TO_TEXT_INSCRIPTION);
      match number_brc {
          Ok(number_brc_c)=>{
            match number_brc_c.iter()?.rev().next() {
              Some((number, _id)) => {
                num = number.value();
              },
              None => {
  ...
  pub (crate) fn get_brc_number_len(&self) -> u64{
    let rtx = self.database.begin_read().unwrap();
    let number_brc = rtx.open_table(NUMBER_TO_TEXT_INSCRIPTION).unwrap();
    let len = number_brc.len().unwrap() as u64;
    len
  }
  //M   
  pub (crate) fn clear_brc_number(
    &self
  ) -> Result<u64> {
    let len = self.get_brc_number_len();
    let txn = self.database.begin_write().unwrap();
    {
        let mut table = txn.open_table(NUMBER_TO_TEXT_INSCRIPTION).unwrap();
        for i in 0..len {
            table.remove(i).unwrap();
        }
    }
    txn.commit().unwrap();
    let len2 = self.get_brc_number_len();
    self.update_brc_number(0).unwrap();
    Ok(len2)
  }
  //M 
  pub (crate) fn scan_text_inscription(
    &self,
    size:usize,
  ) -> Result<(u64,u64)> {
    let last_inscription_number = self.get_inscription_number().unwrap();
    let last_brc_number = self.get_brc_last_number().unwrap();
    let mut to_inscription_number:u64 = 0;
    let mut latest = self.get_text_inscriptions_number().unwrap();
    if last_brc_number < last_inscription_number {
      let rtx = self.database.begin_read()?;
      
      let inscription_number_to_inscription_id =
      rtx.open_table(INSCRIPTION_NUMBER_TO_INSCRIPTION_ID)?;
      let inscriptions:Vec<(u64,InscriptionId)> = inscription_number_to_inscription_id
      .range(last_brc_number..)?
      //.rev()
      .take(size)
      .map(|(number, id)| (number.value(),Entry::load(*id.value())))
      .collect();
      
      for (number,inscription)  in  inscriptions{
          let info = self.get_inscription_info(inscription);
          if info {
            self.insert_brc_inscription(latest,inscription).unwrap();
            latest += 1;
          }
          to_inscription_number = number;
      }
      if to_inscription_number > 0 {
        self.update_brc_number(to_inscription_number).unwrap();
      }
       
    }
    Ok((to_inscription_number,latest))
  }
  //M 
  pub (crate) fn get_inscription_info(
    ...
    let inscription = self.get_inscription_by_id(inscription_id);
    match inscription {
        Ok(v) => {
          let ins = v.unwrap();
          match ins.media() {
            Media::Text => {
              let content = ins.body();
              match content {
                  Some(c) =>{
                    if c.len() > 15 && c[0..14] == INSCRIPTION_TEXT_FILTERATE {
                      return true;
                    }else{
                      return false;
                    }
                    //let txt = str::from_utf8(c);
                  },
                  None => {
                    return false;
                  }
            ...
            Media::Audio =>  {return false;},
            Media::Image =>  {return false;},
            Media::Iframe =>  {return false;},
            Media::Pdf =>  {return false;},
            Media::Unknown =>  {return false;},
            Media::Video =>  {return false;},
          }
        },
        Err(_) => {
          return false;
  ...
  //M 
  pub (crate) fn insert_brc_inscription(
    &self,
    key:u64,
    inscription_id:InscriptionId
  ) -> Result<()> {
    let wtx = self.begin_write()?;
    wtx.open_table(NUMBER_TO_TEXT_INSCRIPTION)?
      .insert(key, &inscription_id.store())?;
    wtx.commit()?;
    Ok(())
  }
