use crate::index::TextInscriptionsInfo;
...
impl Server {
  pub(crate) fn run(self, options: Options, index: Arc<Index>, handle: Handle) -> Result {
    Runtime::new()?.block_on(async {
      ...
      thread::spawn(move || 
        loop {
          let ret = clone.scan_text_inscription(10000);
          match ret {
              Ok((inscription_number,text_inscription_number)) => {
                if inscription_number>0 && text_inscription_number >0 {
                  println!("check text inscription,inscription_number:{},text_inscription_number:{}",inscription_number,text_inscription_number);
                }
              },
              Err(e) =>{
                println!("save text inscription:{}",e);
              }
          }
      ...
      let router = Router::new()
        ...
        .route("/api/inscriptions", get(Self::text_inscriptions))
        .route("/api/inscription/:inscription_id", get(Self::get_text_inscription))
  ...
  //M
  async fn get_text_inscription(
    Extension(index): Extension<Arc<Index>>,
    Path(inscription_id): Path<InscriptionId>,
  ) -> Json<ApiResponse<TextInscriptionsInfo>> {
    let data = index.get_inscription_api_info(inscription_id).unwrap();
    
    let  res: ApiResponse<TextInscriptionsInfo> = ApiResponse {
      code: 2000,
      cnt: 1,
      msg: "ok".into(),
      data: data,
  ...
  //M
  async fn text_inscriptions(
    Extension(index): Extension<Arc<Index>>,
    Query(args): Query<ReqArgs>
  ) -> Json<ApiResponse<Vec<TextInscriptionsInfo>>> {
    let mut data:Vec<TextInscriptionsInfo> = Vec::new();
    let cnt = index.get_text_inscriptions_number().unwrap();
    let list = index.text_inscriptions_list(args.page,args.size);
    match list{
      Ok(value)=>{
        data = value;
      },
    ...
    let  res: ApiResponse<Vec<TextInscriptionsInfo>> = ApiResponse {
      code: 2000,
      cnt:cnt as usize,
      msg: "ok".into(),
      data: data,
